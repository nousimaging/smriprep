...
class AddVol(SimpleInterface):
    ...
    def _run_interface(self, runtime):
        #load in img data
        in_img = nb.load(self.inputs.in_file)
        op_img = np.array(nb.load(self.inputs.operand_file).get_fdata())

        in_data = np.array(in_img.get_fdata())

        #add together
        out_data = in_data + op_img
...
class MakeRibbon(SimpleInterface):
    ...
    def _run_interface(self, runtime):
        ...
        #apply op img as mask to input (using fslmaths rules)
        #mask based on op img >0, i.e. set input to 0 where op is 0 or less
        in_data[op_img<=0] = 0
